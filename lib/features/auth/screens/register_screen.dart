...
class _RegisterScreenState extends State<RegisterScreen>
  ...
  // Internal value stored lowercase (matches UserModel.role values
  // used everywhere else: 'student', 'instructor', 'coordinator').
  ...
  Future<void> _register() async {
    ...
    final selectedCampus = _selectedCampus;

    // Campus is required (validator above already checks this via
    // the form, but this guards against a null value at call time).
    if (selectedCampus == null || selectedCampus.trim().isEmpty) {
      _showMessage('Please select your campus/branch.');
      return;
    }
    ...
    try {
      // CHANGED: the coordinator-per-campus uniqueness check now
      // happens atomically INSIDE AuthService.registerUser (it
      // needs the account to be signed in first, and doing it
      // there avoids a race condition between the check and the
      // actual account creation). If the campus's coordinator slot
      // is already taken, registerUser throws and everything it
      // created gets rolled back automatically.
      ...
      // ==========================================================
      // CHANGED: navigate straight to the correct dashboard based
      // on the selected role, instead of just popping back to login.
      // ==========================================================
      ...
      _showMessage('Firebase error: ${e.message ?? e.code}');
    } catch (e) {
      if (!mounted) return;

      // ADDED: friendlier message for the coordinator-slot-taken
      // case (thrown by AuthService.registerUser as a plain
      // Exception with this text).
      final message = e.toString();

      if (message.contains('coordinator already exists')) {
        _showMessage(
          'A coordinator already exists for this campus. '
        ...
        children: [
          ...
          Row(
            children: _roleOptions.map((option) {
              final bool selected = _selectedRole == option['value'];

              return Expanded(
                child: Padding(
                  padding: EdgeInsets.only(
                    right: option == _roleOptions.last ? 0 : 8,
                  ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // ==========================================================
          // ADDED: CAMPUS / BRANCH DROPDOWN (fixed list)
          // ==========================================================
          DropdownButtonFormField<String>(
            initialValue: _selectedCampus,
            validator: _validateCampus,
            decoration: const InputDecoration(
