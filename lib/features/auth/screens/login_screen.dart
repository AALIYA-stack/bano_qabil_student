...
class _LoginScreenState extends State<LoginScreen>
  ...
  Future<void> _login() async {
    ...
    try {
      final userProfile = await AuthService.instance.login(
      ...
      final role = userProfile.role.trim().toLowerCase();
      ...
      switch (role) {
        ...
        default:
          ...
          if (!mounted) return;
          _showError('Invalid account role. Please contact the coordinator.');
      }
      ...
      if (!mounted) return;
      _showError('Login error: ${e.toString().replaceAll('Exception: ', '')}');
    } finally {
      if (mounted) {
        setState(() {
          _isLoading = false;
  ...
  String _firebaseAuthMessage(FirebaseAuthException exception) {
    switch (exception.code) {
      ...
      case 'app-not-authorized':
        return 'This app is not authorized for the project.';

      case 'invalid-api-key':
        return 'Firebase API key is invalid.';

      default:
        return 'Auth error: ${exception.message ?? exception.code}';
    }
  }
  ...
  Future<void> _forgotPassword() async {
    ...
    try {
      ...
      if (!mounted) return;
      _showError('Password reset error: $e');
    }
  }
          ...
          children: [
            Row(
              children: [
                ...
                Text('Demo Accounts', style: AppTextStyles.heading3),
              ],
            ),
            const SizedBox(height: 6),
            const Text(
  ...
  Widget _demoTile({
    ...
    return InkWell(
      onTap: () {
        _fillDemoAccount(email, password);
      },
